...
async def analyze(request: AnalyzeRequest, session: Session = Depends(get_session)) -> AnalyzeResponse:
    ...
    for url in request.urls:
        ...
        if not get_analysis_by_hash(session, url_hash):
            ...
            save_analysis(session, url_hash, graph, title=title, original_url=url, source_type="url")
...
async def analyze_test() -> AnalyzeResponse:
    ...
    test_data = Graph(
        nodes=[
            Node(id="n1", label="AI-first 架构思维", type=NodeType.main, priority=Priority.critical, reason="课程核心思想", source="text1", description="从传统性能/功能评估转向AI友好性评估"),
            Node(id="n2", label="选择 FastAPI", type=NodeType.main, priority=Priority.critical, reason="AI友好框架选择", source="text1", description="选择 FastAPI 作为 AI 友好框架"),
            Node(id="n3", label="自动文档生成", type=NodeType.dependency, priority=Priority.critical, reason="FastAPI特性", source="text1", description="FastAPI 的自动文档生成特性"),
            Node(id="n4", label="严格数据契约", type=NodeType.dependency, priority=Priority.critical, reason="FastAPI特性", source="text1", description="FastAPI 的严格数据契约特性"),
            Node(id="n5", label="Python 类型系统", type=NodeType.dependency, priority=Priority.optional, reason="技术依赖", source="text1", description="利用 Python 的类型系统"),
            Node(id="n6", label="Cursor AI 编辑器", type=NodeType.dependency, priority=Priority.optional, reason="开发工具", source="text1", description="使用 Cursor AI 编辑器进行开发"),
            Node(id="n7", label="创建第一个应用", type=NodeType.main, priority=Priority.critical, reason="实践步骤", source="text1", description="创建第一个 FastAPI 应用"),
            Node(id="n8", label="定义路由", type=NodeType.main, priority=Priority.critical, reason="实践步骤", source="text1", description="定义 API 路由"),
            Node(id="n9", label="数据验证", type=NodeType.main, priority=Priority.critical, reason="实践步骤", source="text1", description="使用 Pydantic 进行数据验证"),
            Node(id="n10", label="Pydantic", type=NodeType.dependency, priority=Priority.critical, reason="技术依赖", source="text1", description="Pydantic 数据验证库"),
        ],
        edges=[
            Edge(source="n1", target="n2", type=EdgeType.depends_on, reason="架构思维指导框架选择"),
            Edge(source="n2", target="n3", type=EdgeType.implements, reason="FastAPI 提供自动文档生成"),
            Edge(source="n2", target="n4", type=EdgeType.implements, reason="FastAPI 提供严格数据契约"),
            Edge(source="n2", target="n5", type=EdgeType.depends_on, reason="基于 Python 类型系统"),
            Edge(source="n2", target="n6", type=EdgeType.supports, reason="与 Cursor AI 编辑器配合"),
            Edge(source="n1", target="n7", type=EdgeType.depends_on, reason="架构思维指导应用创建"),
            Edge(source="n7", target="n8", type=EdgeType.depends_on, reason="应用需要定义路由"),
            Edge(source="n8", target="n9", type=EdgeType.depends_on, reason="路由需要数据验证"),
            Edge(source="n9", target="n10", type=EdgeType.depends_on, reason="使用 Pydantic 进行验证"),
        ],
...
async def add_url(request: URLInput, session: Session = Depends(get_session)) -> AnalyzeResponse:
